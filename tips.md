...
## 数式入力のショートカット(Word)
2024年4月ごろのアップデートでなぜか使えなくなってしまったようなので、  
下記の要領で再設定する。
1. 左上「ファイル」タブを選択．
1. 左下「オプション」を選択．
1. 左のメニューの「リボンのユーザー設定」を選択．
1. 左下にあるショートカット キー「ユーザー設定」を選択．
1. 分類を「すべてのコマンド」(下から6つ目です) に指定し，コマンドを「EquationToggle」に指定．
1. 下の「キー ストロークの指定」の「割り当てるキーを押してください」欄を選択し，「Alt」+「Shift」+「-」を同時に押す．
1. 左の「現在のキー」欄に「Alt+=」が表示されたのを確認して，左下の「割り当て」を選択．

https://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q11297227991
